...
int main(int argc, char* argv[]) {
    ...
    test.make_test("Merge overlapping regions", [&test]() {
        ssize_t id = 0;
    ...
    test.make_test("Find, get, etc.", [&test]() {
        ssize_t id = 0;
        Gene gene(++id, 1, MergeType::EXTEND);
        for (auto region : test.records) {
            gene.insert(region);
    ...
    test.make_test("Compare gene locations", [&test]() {
        ssize_t id = 0;
        Gene gene(++id, 1, MergeType::EXTEND);
        for (auto region : test.records) {
            gene.insert(region);
    ...
    test.make_test("Filter regions", [&test]() {
        ssize_t id = 0;
        Gene gene(++id, 1, MergeType::EXTEND);
        for (auto region : test.records) {
            gene.insert(region);
    ...
    test.make_test("Confirm with single long region", [&test]() {
        // Initial gene
        ssize_t id = 0;
        Gene gene(++id, 1, MergeType::EXTEND);
        // Exon 0
        gene.insert(Record(9, 15, 1, 1));
        // Exon 1
