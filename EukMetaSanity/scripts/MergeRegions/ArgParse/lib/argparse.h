...
bool validate(T& ref, Validator& validator) { 
    ...
    if (!v) {
        std::cerr << std::endl << ref << " error: " 
            << validator.ERR << std::endl;
    }
    return v;
}
...
struct DefaultValidator {
    template <typename T>
    bool operator()(T& ref) { return true; }
...
class ArgParse {
    public:
        ...
        ArgParse(int& argc, char** argv, const char* d = "ArgParse program!"):
                _description(d), help_requested(false) {
            ...
            if (args.size() == 0) help_requested = true;
        }
        // Pass reference to variable to fill
        template <typename T, typename Validator = DefaultValidator>
        void refer(T& ref, std::initializer_list<const char*> flags, 
            const char* description, Type ptype = Type::DEFAULT, 
            bool required = true, Validator& validator = default_validator);
    ...
    private:
        ...
        template <typename T, typename Validator>
        void flagged(T& ref, Token& data, Type& ptype,
            std::initializer_list<const char*>& it, Validator& val);
...
void ArgParse::parse_or_exit() {
    std::stack<Token> tmp;
    while (!tokens.empty()) {
        tmp.push(tokens.top()); tokens.pop();
    }
    tokens = tmp;
    if (help_requested) parse_fail("");
...
template <typename T, typename Validator>
void ArgParse::refer(T& ref, std::initializer_list<const char*> flags, 
        const char* description, Type ptype, bool required,
        Validator& validator) {
...
void ArgParse::positional(T& ref, Token& data, Validator& validator) {
    ...
    // Add to tokens stack
    tokens.push(data);
    while (!tmp.empty()) { args.push(tmp.top()); tmp.pop(); }
}

template <typename T, typename Validator>
void ArgParse::flagged(T& ref, Token& data, Type& ptype,
            ...
            if (top == *it) {
                ...
                switch (ptype) {
                    ...
                    case Type::FLAG:
                        ...
                        if (args.empty()) parse_fail(ArgParse::ERR);
...
void ArgParse::get_help(const std::string& err_msg) {
    ...
    if (_description.size() > 0) std::cerr << _description << "\n" << std::endl;
    ...
    std::cerr << std::endl;
    std::cerr << "\t\t\t" << it.description << std::endl;
    if (err_msg.size() > 0) std::cerr << err_msg << std::endl;
}
}
