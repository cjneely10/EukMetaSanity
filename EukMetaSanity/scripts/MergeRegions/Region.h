...
#ifndef MERGEREGIONS_REGION_H
#define MERGEREGIONS_REGION_H
...
struct Region {
    Region(const size_t& s, const size_t& e, const ssize_t& st, const size_t& off, const size_t& ct = 1)
            : is_terminal(false),
              ...
              _count(ct) {
        if (start >= end)
            throw RangeException(std::string("Invalid region formed from " + s) + std::string(" to " + e));
    }
    ...
    friend std::ostream& operator<<(std::ostream& o, const Region& rhs) {
        ...
        for (size_t i = 0; i < log10(rhs.end - rhs.start); i++)
            o << '|';
        return o;
    }
