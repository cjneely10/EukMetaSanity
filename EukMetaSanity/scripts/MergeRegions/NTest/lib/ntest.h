...
class NTest {
    public:
        ...
        virtual void test() {
            ...
            if (throw_on_fail) check_method = [](bool _check){ 
                if (!_check) throw std::exception();
            };
            for (auto& test: tests) {
                // Name of test
                std::string& test_name = std::get<0>(test);
                std::cerr << CYAN << "***** Begin " << test_name 
                    << " *****" << RESET << std::endl;
                // Run test and output result
                bool result;
                double _time = NTest::experiment(
                    [&result, &test](){
                        result = std::get<1>(test)();
                    }
                );
                if (!result) {
                    std::cerr << RED << test_name << " failed in " 
                        << _time << "ms" << RESET << std::endl;
                    // Raise error and exit
                    check_method(false);
                } else {
                    std::cerr << GREEN << test_name << " passed in " 
                        << _time << "ms" << RESET << std::endl;
                    _passed++;
                }
                std::cerr << std::endl;
        ...
        void make_test(std::string name, std::function<bool()> test) {
            push(std::make_tuple(name, test));
        }
        ...
        static bool expect(bool a, bool _throw = false, bool b = true) {
            bool _true = (a == b);
            if (!_true &&  _throw) throw NTestExpectError();
            return _true;
        }
    ...
    protected:
        ...
        void push(std::tuple<std::string, std::function<bool()>> test) {
